...
#,amjuel='amjuel.tex',hydhel='hydhel.tex',h2vibr='h2vibr.tex',ADAS='ic#h0-1.dat',UE='ehr1.dat'):
            ...
            if 'XvY' in ID:
                ...
                for x in range(vmax+1):
                    for y in [-1,1]:
                        if x+y in range(vmax+1):
                            ...
                            self.reactions.append(self.REACTION(    ID,
                                                                    database,
                    ...
                    if fit=='RATE':
                        ...
                        if 'X' in name:
                            for j in range(vmax+1):
                                self.reactions.append(self.REACTION(    XY2num(name,j),
                                                                        database_use,
                        ...
                        if 'X' in name:
                            ...
                            self.reactions.append(self.REACTION(    name,
                                                                    database_use,
                                                                    reactants.split(' + '),
                                                                    fragments.split(' + '),
                                                                    float(data[subc+2]),
                ...
                for x in range(1,nmax+1):
                    if ID.upper()=='EXCITATION':
                        ...
                        Tarr=0
                    for y in rn:
                            self.reactions.append(self.REACTION(    '{}_{}-{}'.format(ID,x,y),
                                                                    database,
    ...
    def gl_nt(self,Te,ne,t,n=None,Sext=True):
    ...
    def solve_dt(self,t,mat,ext,n0):
        ''' Time-dependently solves the linearized problem '''
        from scipy.linalg import expm
        from scipy.integrate import quad
        from numpy import matmul,zeros
        #return -matmul(mat,0*ext)+ matmul(expm(mat*t),(n0+matmul(mat,0*ext)))
        #return matmul(expm(mat*t),n0)+ext
        def exsource(x,t,A,ext):
            from scipy.linalg import expm
            from numpy import matmul
            return matmul(expm(A*(t-x)),ext)
        
    

        return matmul(expm(mat*t),n0)#+quad(exsource,0,t,args=(t,mat,ext))

    def nt(self,tarr,Te,ne,Ti=None,ni=None,E=0.1,n0=None,V=1):
        ''' Returns a NxNt matrix with the dt-solution for n, from n0 '''
        from numpy import zeros
        if n0 is None:
            n0=self.n0
        if Ti is None:
        ...
        nt=zeros((len(self.species),len(tarr)))
        ntot=zeros(tarr.shape)
        mat=self.M(Te,ne,Ti,ni,E)
        
        for t in range(len(tarr)):
            nt[:,t]=self.solve_dt(tarr[t],*mat,n0)
            ntot[t]=self.totparticles(nt[:,t],V)
        return nt,ntot



        
    def ntg(self,tarr,Te,ne,Ti=None,ni=None,E=0.1,n0=None,V=1):
        ''' Returns a NxNt matrix with the dt-solution for n, from n0 '''
        from numpy import zeros
        if n0 is None:
            n0=self.n0
        ...
        nt=zeros((self.Np,len(tarr)))
        ntot=zeros(tarr.shape)
        mat=self.M(Te,ne,Ti,ni,E)
        
        for t in range(len(tarr)):
            nt[:,t]=self.greenland_dt(tarr[t],*mat,n0)
            ntot[t]=self.totparticles(nt[:,t],V)
        return nt,ntot
    ...
    def greenland_dt(self,t,mat,ext,n0):
        ''' Creates a rate matrix for the P-space according to Greenland 2001 '''
        from numpy import zeros,matmul,diag,sum,log10
        from numpy.linalg import inv,eig
        from scipy.linalg import expm
        import scipy.sparse as s

        # TODO: Move to upper level function
        n0=self.n0
        ...
        eigs,T=eig(mat)
        origdiag=matmul(matmul(T,diag(eigs)),inv(T))
        eigind=eigs.argsort()[::-1]
        eigs=eigs[eigind]
        T=T[:,eigind]
        D=diag(eigs)
        ...
        GPp=GP-matmul(matmul(Delta,inv(TQ)),GQ)
        ...
        nP0=n0[:self.Np]
        nQ0=n0[self.Np:]
        nP0p=nP0-matmul(matmul(Delta,inv(TQ)),nQ0)
        
        #print(eigs)
        #print(matmul(delta,inv(TP)))
        #print(log10(sum(abs(matmul(delta,inv(TP))),axis=1)))
        #print(sum(abs(matmul(delta,inv(TP))),axis=0))
        # TODO
        #return -matmul(Meff,0*GPp)+matmul(expm(Meff*t),(nP0p+matmul(Meff,0*GPp)))
        return matmul(expm(Meff*t),nP0p)#+matmul(expm(-Meff*t),GPp)*t
#        print(matmul(Meff,nP0p)+GP-matmul(matmul(Delta,inv(TQ)),GQ))


    def create_UE_rates(self,fname='uerates',iH2=None,iH0=None):
        # TODO: How to arange data so that it only depends on ne and ni??
        ...
        for i in range(15):
            for j in range(60):
                ret[i,j]=self.M(10**(-1.2 +j/10),10**(10+0.5*j))
        ...
        try:
            ...
            pass
        if iH2 is None:
            iH2=self.species.index('H2(v=0)')
        if iH0 is None:
            iH0=self.species.index('H0(n=1)')
        
        with open('output/{}.dat'.format(fname),'w') as f:
            f.write(' H0 reac rate(jt,jn) (cm**3 s**-1)  Te(jt) = 10**(-1.2 + (jt-1)/10) jt=1,60\n')
            for j in range(15):
                f.write('jn =   {}'.format(j+1)+(j==0)*'; jt = 1 -> 60 going by rows   ne(jn) = 10**(10 + 0.5*(jn-1)) jn=1,15 \n')
                out=''
                for k in range(6):
                    for i in range(10):
                        out.append('  {:1.5E}'.format(ret[j,k*10+i]))
                    out.append('\n')
                f.write(out)
